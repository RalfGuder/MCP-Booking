...
Der Nutzer fragt den KI-Assistenten nach einer Uebersicht von Buchungen (z.B. "Zeige mir alle genehmigten Buchungen fuer naechste Woche").

## Hauptablauf

1. Der KI-Assistent ruft das MCP-Tool `list_bookings` auf.
2. Der MCP-Server validiert die uebergebenen Parameter.
3. Der MCP-Server sendet einen `GET /bookings`-Request an die API mit den Query-Parametern.
4. Die API liefert eine paginierte Liste von Buchungen zurueck.
5. Der MCP-Server transformiert die API-Antwort in ein strukturiertes Tool-Ergebnis.
6. Der KI-Assistent praesentiert die Buchungsliste dem Nutzer.
...
4a. Der MCP-Server liefert eine Fehlermeldung: "Authentifizierung fehlgeschlagen. Bitte API-Zugangsdaten pruefen."

### E2: Berechtigungsfehler (403)
3a. Die API liefert 403 Forbidden.
4a. Der MCP-Server liefert eine Fehlermeldung: "Keine Berechtigung fuer diese Aktion."

### E3: API nicht erreichbar
3a. Der HTTP-Request schlaegt fehl (Timeout, DNS-Fehler).
4a. Der MCP-Server liefert eine Fehlermeldung: "Die Booking API ist nicht erreichbar."

### E4: Ungueltiger Parameter
2a. Ein Parameter hat einen ungueltigen Wert (z.B. status = "invalid").
