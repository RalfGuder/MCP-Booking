...
Der Nutzer moechte eine Ressource aendern (z.B. "Aendere die Kosten der Ressource 3 auf 75 EUR").

## Hauptablauf

1. Der KI-Assistent ruft das MCP-Tool `update_resource` mit ID und den zu aendernden Feldern auf.
2. Der MCP-Server validiert die Parameter.
3. Der MCP-Server sendet einen `PUT /resources/{id}`-Request an die API.
4. Die API aktualisiert die Ressource und liefert eine Bestaetigung.
